...
public class AuthorizationServerLiveTest {
	...
	public void whenServiceStartsAndLoadsRealmConfigurations_thenOidcDiscoveryEndpointIsAvailable() {
		final String oidcDiscoveryUrl = "http://localhost:8083/auth/realms/mytest/.well-known/openid-configuration";
	...
	private String obtainAccessToken() {
		final String redirectUrl = "http://localhost:8083/mytest/login/oauth2/code/custom";
		final String authorizeUrl = "http://localhost:8083/auth/realms/mytest/protocol/openid-connect/auth?response_type=code&client_id=partypatcher&scope=read"
				+ "&redirect_uri=" + redirectUrl;
								 
		final String tokenUrl = "http://localhost:8083/auth/realms/mytest/protocol/openid-connect/token";
		...
		response = RestAssured.given().redirects().follow(false).cookie("AUTH_SESSION_ID", authSessionId)
				.formParams("username", "testusr", "password", "123", "credentialId", "")
